...
archivo_propiedades = './propiedades_007.csv'
...
alpha_ini = 0.45 # Fracción inicial de cenizas
...
# Aca se extraeen los valores de densida del dominio pra poder determinar 
...
array_BVTV = array_densidad_gr_cm3 /(1.41)
print("00__Hay NaN en expression_array_BVTV:", np.isnan(array_BVTV).any())

fem_function_BVTV.x.array[:]= array_BVTV
                        

with XDMFFile(mesh.comm, "./21_06_24/fem_function_BVTV.xdmf", "w") as file:
          file.write_mesh(mesh)
          file.write_function(fem_function_BVTV ,0.0)



condicion_cortical= array_BVTV >= 0.7
condicion_trabecular = array_BVTV < 0.7
...
array_nu = np.empty_like(array_BVTV)
array_formacion_vb = np.zeros_like(array_BVTV)
array_young_dot = np.empty_like(array_BVTV)
array_nu[condicion_cortical] = 0.32
array_nu[condicion_trabecular] =0.2
...
array_modulo_E_0 = 84370*(array_BVTV ** 2.58) * (alpha_ini ** 2.74)*1e3
...
fem_function_densidad = fem.Function(W,name="densidad")
fem_function_vr_dot = fem.Function(W,name="osteoclast_activity")
fem_function_vb_dot = fem.Function(W,name="osteoblast_activity")
fem_function_densidad_gr_cm3 = fem.Function(W,name= "densidad")
fem_function_array_psi= fem.Function(W, name="array_psi")
fem_function_array_psi_reference = fem.Function(W,name="psi_reference")
...
fem_function_expression_array_dot_tbr = Function(W,name="rdot_tbr")
fem_function_expresion_array_dot_vbr =Function(W,name= "rdot_vbr")
...
# ----------------------------------------------------------------------------------
# with XDMFFile(mesh.comm, "./21_06_24/fem_function_modulo_E_0.xdmf", "w") as file:
#           file.write_mesh(mesh)
#           file.write_function(fem_function_modulo_E_f ,0.0)


# with XDMFFile(mesh.comm, "./21_06_24/fem_function_densidad_0.xdmf", "w") as file:
#           file.write_mesh(mesh)
...
def dot_young(Escalar_Beta, array_rho_punto,sigma):
    return Escalar_Beta*array_rho_punto * ufl.outer(sigma(v),sigma(v)) /array_densidad * ufl.inner(sigma,epsilon(u))
...
# +
uh = Function(V, name="desplazamiento_mm")
...
## 
## Escalar_Beta*array_rho_punto * ufl.outer(sigma(v),sigma(v)) /array_densidad * ufl.inner(sigma,epsilon(u))
#fem_function_modulo_E_f.x.array[:] = array_young_dot
...
#array_young_dot = escalar_Beta*array_rdot*ufl.outer(sigma(uh),sigma(uh)) /array_densidad * ufl.inner(sigma(uh),epsilon(uh))
...
U = 0.5 * ufl.inner(sigma(uh),epsilon(uh)) * ufl.dx
...
#---------------------------------------------------------------------------------
expression_array_dot_tbr = np.zeros(W.dofmap.index_map.size_local)
...
array_alpha.fill(0.45)
array_vm = np.empty_like(array_BVTV)
array_BVTV = array_densidad_gr_cm_3/(1.41 )    # <<<<-----------
array_porosidad = 1 - array_BVTV
...
# --------------------------------------------------------------------------------------------
# Aca inicia fase 1  esta etapa es de reabsortion  REABSORTION
with XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_modulo.xdmf", "w") as file_modulo, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_BVTV.xdmf", "w") as file_BVTV, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_vm.xdmf", "w") as file_array_vm, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_damage.xdmf", "w") as file_array_damage, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_alpha.xdmf", "w") as file_alpha, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi.xdmf", "w") as file_array_psi, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_error.xdmf", "w") as file_array_error, \
     XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_densidad_fase_1.xdmf", "w") as file_densidad_aparente:

    # Escribe la malla una vez
     file_dot_tbr.write_mesh(mesh)
     file_dot_vbr.write_mesh(mesh)
     file_densidad_material.write_mesh(mesh)
     file_modulo.write_mesh(mesh)
     file_BVTV.write_mesh(mesh)
     file_array_modulo.write_mesh(mesh)
     file_array_vm.write_mesh(mesh)
     file_array_damage.write_mesh(mesh)
     file_alpha.write_mesh(mesh)
     file_array_rdot.write_mesh(mesh)
     file_array_psi.write_mesh(mesh)
     file_array_psi_reference.write_mesh(mesh)
     file_array_error.write_mesh(mesh)
     file_densidad_aparente.write_mesh(mesh)
...
for dmes in range(1, 2):
    ...
    for dday in range(1, 25):  # Ciclo externo con dtime va de 1 a 24
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            array_psi_reference = array_psi - (array_psi - 50)*math.exp(-0.002*escalar_contador_total_dias_modelo)

            fem_function_array_psi.x.array[:]= array_psi
            fem_function_array_psi_reference.x.array[:] = array_psi_reference
            fem_function_array_psi_reference.x.scatter_forward()
            # ---------------------------------------------------------------------------------------------------------
            # Cálculo del error.
            # ---------------------------------------------------------------------------------------------------------
            array_error= ((1/array_BVTV)**2)*array_psi - array_psi_reference
            fem_function_array_error.x.array[:]= array_error
            fem_function_array_error.x.scatter_forward()
            ...
            fem_function_array_rdot.x.scatter_forward()
            ...
            expression_array_dot_vr[condicion_cortical] = array_BVTV[condicion_cortical] * array_derivative_NBMU[condicion_cortical] * escalar_tmp_ar1 * (t_prima_24 /escalar_TR)**2 
            
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # Revision de fase 1 en remodelación trabecular -----------------------  xxxxxx ------------------------------- atento al signo negativo de carter
            # aca debo entender que hay que  restarle al trabecular por que la funcion propuesta por Carter crea el hueso.no lo remodela es por esto que la funcion 
            # trabecular la prueba consiste en asignarle un valor negativo para que lo reabsorba eso indica que siempre esta formando hueso.
            # -----------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------

            #                                                                  xxxxx
            array_BVTV[condicion_trabecular] = array_BVTV[condicion_trabecular] - expression_array_dot_tbr[condicion_trabecular] / escalar_TR
            ...
            array_densidad = 1.41 * array_BVTV #<<<<----------------
            array_densidad_material = 1.41  #<<<<----------------
            fem_function_densidad_material.x.array[:] = array_densidad_material
            fem_function_densidad_material.x.scatter_forward()
            ...
            fem_function_vr_dot.x.scatter_forward()
            
            fem_function_densidad.x.array[:] = array_densidad*1e-6
            fem_function_densidad.x.scatter_forward()
            
            fem_function_densidad_gr_cm3.x.array[:] = array_densidad
            fem_function_densidad_gr_cm3.x.scatter_forward()
            
            fem_function_BVTV.x.array[:] = array_BVTV
            fem_function_BVTV.x.scatter_forward()
            
            fem_function_modulo_E_f.x.array[:] = array_modulo_E_f
            fem_function_modulo_E_f.x.scatter_forward()
            fem_function_damage.x.array[:] = array_damage
            fem_function_damage.x.scatter_forward()
            fem_function_alpha.x.array[:] = array_alpha
            fem_function_alpha.x.scatter_forward()
            ...
            U = 0.5 * ufl.inner(sigma(uh),strain(uh)) * ufl.dx
           ...
           # ----------------------------------------------------------------------------------------------------------
    file_dot_tbr.write_function(fem_function_expression_array_dot_tbr,dday)
    file_dot_vbr.write_function(fem_function_expresion_array_dot_vbr,dday)
    file_densidad_material.write_function(fem_function_densidad_material,dday)
    file_modulo.write_function(fem_function_modulo_E_f,dday)
    file_BVTV.write_function(fem_function_BVTV,dday)
    file_array_modulo.write_function(fem_function_modulo_E_f,dday)
    file_array_vm.write_function(fem_function_array_vm,dday)
    file_array_damage.write_function(fem_function_damage,dday)
    file_alpha.write_function(fem_function_alpha,dday)
    file_array_rdot.write_function(fem_function_array_rdot,dday)
    file_array_psi.write_function(fem_function_array_psi,dday)
    file_array_psi_reference.write_function(fem_function_array_psi_reference,dday)
    file_array_error.write_function(fem_function_array_error,dday)
    file_densidad_aparente.write_function(fem_function_densidad_gr_cm3,dday)
...
# ----------------------------------------------------------------------------------------------------
# aca inicia fase 2 en fase 2 continua reabsorbiendo
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_2/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#  git@github.com:PedroJose00/100_Materials.git
#---------------------------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------------------
    ...
    for dday in range(1, 8):  # Ciclo externo con dtime va de 1 a 8
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # Revision de fase 2 en remodelación trabecular -----------------------  xxxxxx ------------------------------- atento al signo negativo de carter
            # aca debo entender que hay que  restarle al trabecular por que la funcion propuesta por Carter crea el hueso.no lo remodela es por esto que la funcion 
            # trabecular la prueba consiste en asignarle un valor negativo para que lo reabsorba eso indica que siempre esta formando hueso.
            # -----------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------------------
# Aca inicia fase 3 en fase 3 continua Formacion
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_3/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ----------------------------------------------------------------------------------------------------
# aca inicia fase 4 en fase 4 continua Formacion
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_4/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#----------------------------------------------------------------------------------------------
    for dday in range(1, 5):  # Ciclo externo con dtime va de 1 a 5
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            #-------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # Revision de fase 4 en remodelación trabecular -----------------------  xxxxxx ------------------------------- atento al signo negativo de carter
            # aca debo entender que hay que  restarle al trabecular por que la funcion propuesta por Carter crea el hueso.no lo remodela es por esto que la funcion 
            # trabecular la prueba consiste en asignarle un valor negativo para que lo reabsorba eso indica que siempre esta formando hueso.
            # -----------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
            ...
            array_BVTV[condicion_trabecular] = array_BVTV[condicion_trabecular] - expression_array_dot_tbr[condicion_trabecular] + array_derivative_NBMU[condicion_trabecular]
           ...
           # print("Sigma_bar:", sigma_bar_i)
            ...
            print("#-------------------------------------------------------------------------------------------------")
# ----------------------------------------------------------------------------------------------------
# Aca inicia fase 5 en fase 3 continua Formacion  6 dias restantes 
# ----------------------------------------------------------------------------------------------------
    file_dot_tbr.write_function(fem_function_expression_array_dot_tbr,dday)
    file_dot_vbr.write_function(fem_function_expresion_array_dot_vbr,dday)
    file_densidad_material.write_function(fem_function_densidad_material,dday)
    ...
    with XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_5/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#-------------------------------------------------------------------------------------------
    ...
    for dday in range(1, 7):  # Ciclo externo con dtime va de 1 a 24
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # Revision de fase 5 en remodelación trabecular -----------------------  xxxxxx ------------------------------- atento al signo negativo de carter
            # aca debo entender que hay que  restarle al trabecular por que la funcion propuesta por Carter crea el hueso.no lo remodela es por esto que la funcion 
            # trabecular la prueba consiste en asignarle un valor negativo para que lo reabsorba eso indica que siempre esta formando hueso.
            # -----------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
...
#            print(f" dentro de la quinta etapa -->> La expression_array_dot_vr: { escalar_tmp_ar5}")
            ...
            array_formacion_vb[condicion_trabecular]= array_formacion_vb[condicion_trabecular] - (expression_array_dot_tbr[condicion_trabecular]/escalar_TR)
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
            ...
            def strain(u):
                return ufl.sym(ufl.grad(u))  # Equivalent to 0.5*(ufl.nabla_grad(u) + ufl.nabla_grad(u).dday)
            a = form(inner(sigma(u), grad(v)) * dx)
            L = form(inner(f, v) * dx)
...
# ----------------------------------------------------------------------------------------------------
# Aca termina fase 5 en fase 3 continua Formacion  6 dias restantes 
# ----------------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_51/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
# ------------------------------------------------------------------------------------
    print("---------------------------------------------------------")              
    print("El valor promedio de la BVTV es:", promedios_BVTV[escalar_contador_total_dias])
    print(f" terminan la quinta etapa  18 dias --->> La energía: {energy}")
    print("---------------------------------------------------------")              
    tmp_lenght = escalar_contador_total_dias
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
    ...
    with XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_6/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
    ...
    print("#-------------------------------------------------------------------------------------------------")
#----------------------------------------------------------------------------------------
    # Para fase #7 Esta fase dura 64 dias + TR + TI  Aca solo hay formacion
#-------------------------------------------------------------------------------------------
    with XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_7/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#            array_modulo_E_f[condicion_cortical]= 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
    ...
    with XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_dot_tbr.xdmf", "w") as file_dot_tbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_dot_vbr.xdmf", "w") as file_dot_vbr, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_densidad_material.xdmf", "w") as file_densidad_material, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_modulo.xdmf", "w") as file_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_BVTV.xdmf", "w") as file_BVTV, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_modulo.xdmf", "w") as file_array_modulo, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_vm.xdmf", "w") as file_array_vm, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_damage.xdmf", "w") as file_array_damage, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_alpha.xdmf", "w") as file_alpha, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_rdot.xdmf", "w") as file_array_rdot, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_psi.xdmf", "w") as file_array_psi, \
         XDMFFile(mesh.comm, "./21_06_24/fase_1/fem_function_array_psi_reference.xdmf", "w") as file_array_psi_reference, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_array_error.xdmf", "w") as file_array_error, \
         XDMFFile(mesh.comm, "./21_06_24/fase_71/fem_function_densidad_aparente.xdmf", "w") as file_densidad_aparente:
...
#   -------------------------------------------------------------------------------------------------------------
    ...
    for dday in range(1, 5):  # Ciclo externo con dtime va de 1 a 24
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            # ----------------------------------------------------------------------------------------------------------------------------------------
            # Revision de fase 7.1 en remodelación trabecular -----------------------  xxxxxx ------------------------------- atento al signo negativo de carter
            # aca debo entender que hay que  restarle al trabecular por que la funcion propuesta por Carter crea el hueso.no lo remodela es por esto que la funcion 
            # trabecular la prueba consiste en asignarle un valor negativo para que lo reabsorba eso indica que siempre esta formando hueso.
            # -----------------------------------------------------------------------------------------------------------------------------------
            # ----------------------------------------------------------------------------------------------------------------------------------------
            ...
            array_vm[condicion_trabecular] = (escalar_vm * array_COPY_BVTV[condicion_trabecular] + escalar_vm * (array_formacion_vb[condicion_trabecular] - expression_array_dot_tbr[condicion_trabecular]))/array_BVTV[condicion_trabecular]
            array_vm[condicion_cortical] = (escalar_vm * array_COPY_BVTV[condicion_cortical] + escalar_vm * (array_formacion_vb[condicion_cortical]- expression_array_dot_vr[condicion_cortical]))/array_BVTV[condicion_cortical]
...
data.to_csv('./21_06_24/resultados_promedio_final.csv', index=False)
